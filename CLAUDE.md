...
- A runtime toggle only rewrites the state file; nothing reloads the config, so **everything `monitors.lua`/`windows.lua` declare per mode has to be re-declared by hand** in `monitors-layout-toggle` (`set_monitor`/`disable_monitor` for outputs, `apply_workspace_rules` for workspace rules). Miss one and Hyprland keeps the *other* mode's rules, so nothing pulls ws1-7 onto the TV at all. Unlike `hl.monitor`, `hl.workspace_rule` is keyed by workspace and replaces rather than merges, and `default = false` really does clear the flag.
- **Workspace rules do not decide where a TV switch lands.** `onConnect` applies the `default = true` rule *and then* overrides it with the workspace that output was last active on (`Monitor HDMI-A-1 was on workspace 8, setting it to that` in `hyprland.log`, right after `moveWorkspaceToMonitor: Moving 1 to monitor 0`). That memory is set when the output is disabled. Bringing DP-2 up in the pc branch takes ws1 back and Hyprland plugs the gap on HDMI-A-1 with a fresh workspace (`count + 1` = 8), so disabling it right then made 8 the remembered id and every later TV switch opened on ws8. `monitors-layout-toggle` now moves ws1 back onto HDMI-A-1 and focuses it before `disable_monitor`; `pin_ws1` in `settle_tv` stays as the backstop for a TV that re-adds itself mid-wake. The other ws8 source is the headless **FALLBACK** monitor: the @120 modeset drops the TV off the bus for a moment, Hyprland has no enabled output left and spins up FALLBACK, and when FALLBACK is torn down its own remembered workspace (8) is dumped onto the TV (`Disconnecting Monitor FALLBACK was on workspace 8` → `SWITCHINGISACTIVE, active 1 -> 8`) — after the post-modeset `pin_ws1` has already succeeded. Hence the 15s hold loop instead of a single pin.
